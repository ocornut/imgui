...
// Functions
bool ImGui_ImplGL_Init(const char *glsl_version) {
    ImGuiIO &io = ImGui::GetIO();
    io.BackendRendererName = "imgui_impl_gl";

    // Store GLSL version string so we can refer to it later in case we recreate shaders
#if defined(IMGUI_IMPL_OPENGL_ES_3)
    glsl_version = "#version 300 es";       // No other choice
#elif defined(IMGUI_IMPL_OPENGL_ES_2)
    glsl_version = "#version 100";          // No other choice
#elif defined(IMGUI_IMPL_OPENGL_3)
    if (glsl_version == NULL)
        glsl_version = "#version 130";      // Use GL 3.0 by default
    else {
        // TODO: only 130, 140, 150, 330 should be authorized
    }
#elif defined(IMGUI_IMPL_OPENGL_4)
    if (glsl_version == NULL)
        glsl_version = "#version 410 core";     // Use GL 4.1 by default
    else {
        // TODO: only 400, 410, 420, 430, 440, 450, 460 should be authorized
    }
#else
...
void ImGui_ImplGL_Shutdown() {
    ImGui_ImplGL_DestroyDeviceObjects();
}
...
#if defined(IMGUI_IMPL_OPENGL_ES_2)
    // GL_UNPACK_ROW_LENGTH not supported
#else
...
void ImGui_ImplGL_DestroyFontsTexture() {
    if (g_FontTexture) {
        ImGuiIO &io = ImGui::GetIO();
...
static bool CheckShader(GLuint handle, const char *desc) {
    ...
    if ((GLboolean) status == GL_FALSE)
        fprintf(stderr, "ERROR: ImGui_ImplGL_CreateDeviceObjects: failed to compile %s!\n", desc);
    if (log_length > 0) {
        ImVector<char> buf;
...
static bool CheckProgram(GLuint handle, const char *desc) {
    ...
    if ((GLboolean) status == GL_FALSE)
        fprintf(stderr, "ERROR: ImGui_ImplGL_CreateDeviceObjects: failed to link %s! (with GLSL '%s')\n", desc,
                g_GlslVersionString);
    if (log_length > 0) {
        ImVector<char> buf;
        buf.resize((int) (log_length + 1));
...
bool ImGui_ImplGL_CreateDeviceObjects() {
    ...
    ImGui_ImplGL_CreateFontsTexture();
...
void ImGui_ImplGL_DestroyDeviceObjects() {
    ...
    ImGui_ImplGL_DestroyFontsTexture();
}
