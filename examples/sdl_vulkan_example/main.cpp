...
static void resize_vulkan(SDL_Window* /*window*/, int w, int h)
{
    VkResult err;
...
static void setup_vulkan(SDL_Window* window)
{
    VkResult err;
	...
	// Create Vulkan Instance
    {
...

        {
            ...
            if (!SDL_vk_GetInstanceExtensions(&c, &extension_names[extension_count])) {
	...
	{
		if (!SDL_vk_CreateSurface(window, g_Instance, &g_Surface)) {
...
int main(int, char**)
{
    ...
    SDL_Window *window = SDL_CreateWindow("ImGui SDL2+Vulkan example", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, SDL_WINDOW_RESIZABLE);
    ...
    ImGui_ImplSdlVulkan_Init(window, &resize_vulkan, &init_data);
    ...
    // Upload Fonts
    {
        ...
        ImGui_ImplSdlVulkan_CreateFontsTexture(g_CommandBuffer[g_FrameIndex]);
    ...
    while (!done)
    {
        ...
        frame_begin();
        ImGui_ImplSdlVulkan_Render(g_CommandBuffer[g_FrameIndex]);
        frame_end();
    }
