...
#ifndef NDEBUG
    ImGui_ImplWGPU_PrintAdapterInfo_Helper(acquired_adapter.Get());
#endif
...
static WGPUAdapter RequestAdapter(WGPUInstance& instance)
{
    ...
    static WGPUAdapter local_adapter;
    WGPURequestAdapterCallbackInfo adapterCallbackInfo = {};
    adapterCallbackInfo.callback = handle_request_adapter;
    adapterCallbackInfo.userdata1 = &local_adapter;

    wgpuInstanceRequestAdapter(wgpu_instance, &adapter_options, adapterCallbackInfo);
    IM_ASSERT(local_adapter && "Error on Adapter request");
#ifndef NDEBUG
    ImGui_ImplWGPU_PrintAdapterInfo_Helper(local_adapter);
#endif
...
{
    static WGPUDevice local_device;
...
static bool InitWGPU(SDL_Window* window)
{
...
#else
    wgpu::Surface surface = ImGui_ImplSDL2_CreateWGPUSurface(instance.Get(), window);
#endif
    if (!surface)
        return false;
    ...
    // Create the surface.
    wgpu_surface = ImGui_ImplSDL2_CreateWGPUSurface(wgpu_instance, window);
    if (!wgpu_surface)
        return false;
