...
static ImGuiWindow*     CreateNewWindow(ImStr name, ImVec2 size, ImGuiWindowFlags flags);
...
static ImGuiIniData*    FindWindowSettings(ImStr name);
static ImGuiIniData*    AddWindowSettings(ImStr name);
...
void ImGuiIO::AddInputCharactersUTF8(ImStr utf8_chars)
{
...
const char* ImStrchr(const char* str, const char* str_end, char c)
{
...
static void LogRenderedText(const ImVec2* ref_pos, ImStr text)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;

    if (!text.End)
        text.End = ImGui::FindRenderedTextEnd(text);
...
void ImGui::OpenPopup(ImStr str_id)
{
    ImGuiContext& g = *GImGui;
...
bool ImGui::BeginPopup(ImStr str_id)
{
    ImGuiContext& g = *GImGui;
...
bool ImGui::IsPopupOpen(ImStr str_id)
{
    ImGuiContext& g = *GImGui;
    return g.OpenPopupStack.Size > g.CurrentPopupStack.Size && g.OpenPopupStack[g.CurrentPopupStack.Size].PopupId == g.CurrentWindow->GetID(str_id);
}

bool ImGui::BeginPopupModal(ImStr name, bool* p_open, ImGuiWindowFlags extra_flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::OpenPopupOnItemClick(ImStr str_id, int mouse_button)
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    if (IsMouseClicked(mouse_button) && IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup))
    {
        ImGuiID id = str_id.Begin ? window->GetID(str_id) : window->DC.LastItemId; // If user hasn't passed an ID, we can use the LastItemID. Using LastItemID as a Popup ID won't conflict!
...
bool ImGui::BeginPopupContextItem(ImStr str_id, int mouse_button)
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    ImGuiID id = str_id.Begin ? window->GetID(str_id) : window->DC.LastItemId; // If user hasn't passed an ID, we can use the LastItemID. Using LastItemID as a Popup ID won't conflict!
    IM_ASSERT(id != 0);                                                  // However, you cannot pass a NULL str_id if the last item has no identifier (e.g. a Text() item)
...
bool ImGui::BeginPopupContextWindow(ImStr str_id, int mouse_button, bool also_over_items)
{
    if (!str_id.Begin)
        str_id = ImStr("window_context");
...
static bool BeginChildEx(ImStr name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ...
    if (name.Begin)
    {
        int len = name.End ? (int)(name.End - name.Begin) : strlen(name.Begin);
        title = ImStr(buf, ImFormatString(buf, IM_ARRAYSIZE(buf), "%s/%.*s_%08X", parent_window->Name, len, name.Begin, id));
    }
    else
        title = ImStr(buf, ImFormatString(buf, IM_ARRAYSIZE(buf), "%s/%08X", parent_window->Name, id));
...
bool ImGui::Begin(ImStr name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
            ...
            const ImVec2 text_size = CalcTextSize(name, true);
            ...
            RenderTextClipped(text_min, text_max, name, &text_size, style.WindowTitleAlign, &clip_rect);
        }
...
{
    TextUnformatted(ImStr(text, text_end));
}
...
void ImGui::LabelText(ImStr label, ImStr text)
{
    ...
    // Render
    RenderTextClipped(value_bb.Min, value_bb.Max, text, NULL, ImVec2(0.0f,0.5f));
...
bool ImGui::InvisibleButton(ImStr str_id, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, ImStr label)
{
    ...
    if (display_frame)
    {
        ...
        if (g.LogEnabled)
        {
            ...
            RenderTextClipped(text_pos, bb.Max, label, &label_size);
            LogRenderedText(&text_pos, ImStr(log_suffix+1, log_suffix+3));
        }
        else
        {
            RenderTextClipped(text_pos, bb.Max, label, &label_size);
...
bool ImGui::TreeNodeExV(ImStr str_id, ImGuiTreeNodeFlags flags, const char* fmt, va_list args)
{
    ...
    ImGuiContext& g = *GImGui;
    ImStr label(g.TempBuffer, ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args));
...
{
    ImGuiWindow* window = GetCurrentWindowRead();
    window->IDStack.push_back(window->GetID(ImStr(str_id_begin, str_id_end)));
}
...
void ImGui::BulletText(ImStr text)
{
    ...
    // Render
    RenderBullet(bb.Min + ImVec2(style.FramePadding.x + g.FontSize*0.5f, line_height*0.5f));
    RenderText(bb.Min+ImVec2(g.FontSize + style.FramePadding.x*2, text_base_offset_y), text, false);
}
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect& aabb, ImStr label, ImGuiDataType data_type, void* data_ptr, ImGuiID id, int decimal_precision)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
...
int ImGui::ParseFormatPrecision(const char* fmt, const char* fmt_end, int default_precision)
{
    int precision = default_precision;
    while ((fmt = ImStrchr(fmt, fmt_end, '%')) != NULL)
    {
        fmt++;
...
//   "Gold: %.0f"   Gold: 1
bool ImGui::SliderFloat(ImStr label, float* v, float v_min, float v_max, ImStr display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    char value_buf[64];
    ImStr value = ImStr(value_buf, ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), fmt_buf, *v));
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value, NULL, ImVec2(0.5f,0.5f));
...
bool ImGui::VSliderFloat(ImStr label, const ImVec2& size, float* v, float v_min, float v_max, ImStr display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    char value_buf[64];
    ImStr value = ImStr(value_buf, ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), fmt_buf, *v));
    RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max, value, NULL, ImVec2(0.5f,0.0f));
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::SliderAngle(ImStr label, float* v_rad, float v_degrees_min, float v_degrees_max)
{
...
bool ImGui::SliderInt(ImStr label, int* v, int v_min, int v_max, ImStr display_format)
{
    if (!display_format.Begin)
        display_format.Begin = "%.0f";
    float v_f = (float)*v;
...
bool ImGui::VSliderInt(ImStr label, const ImVec2& size, int* v, int v_min, int v_max, ImStr display_format)
{
    if (!display_format.Begin)
        display_format.Begin = "%.0f";
    float v_f = (float)*v;
...
bool ImGui::SliderFloatN(ImStr label, float* v, int components, float v_min, float v_max, ImStr display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::SliderFloat2(ImStr label, float v[2], float v_min, float v_max, ImStr display_format, float power)
{
    return SliderFloatN(label, v, 2, v_min, v_max, display_format, power);
}

bool ImGui::SliderFloat3(ImStr label, float v[3], float v_min, float v_max, ImStr display_format, float power)
{
    return SliderFloatN(label, v, 3, v_min, v_max, display_format, power);
}

bool ImGui::SliderFloat4(ImStr label, float v[4], float v_min, float v_max, ImStr display_format, float power)
{
    return SliderFloatN(label, v, 4, v_min, v_max, display_format, power);
}

bool ImGui::SliderIntN(ImStr label, int* v, int components, int v_min, int v_max, ImStr display_format)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::SliderInt2(ImStr label, int v[2], int v_min, int v_max, ImStr display_format)
{
    return SliderIntN(label, v, 2, v_min, v_max, display_format);
}

bool ImGui::SliderInt3(ImStr label, int v[3], int v_min, int v_max, ImStr display_format)
{
    return SliderIntN(label, v, 3, v_min, v_max, display_format);
}

bool ImGui::SliderInt4(ImStr label, int v[4], int v_min, int v_max, ImStr display_format)
{
...
bool ImGui::DragFloat(ImStr label, float* v, float v_speed, float v_min, float v_max, ImStr display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::DragFloatN(ImStr label, float* v, int components, float v_speed, float v_min, float v_max, ImStr display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::DragFloat2(ImStr label, float v[2], float v_speed, float v_min, float v_max, ImStr display_format, float power)
{
    return DragFloatN(label, v, 2, v_speed, v_min, v_max, display_format, power);
}

bool ImGui::DragFloat3(ImStr label, float v[3], float v_speed, float v_min, float v_max, ImStr display_format, float power)
{
    return DragFloatN(label, v, 3, v_speed, v_min, v_max, display_format, power);
}

bool ImGui::DragFloat4(ImStr label, float v[4], float v_speed, float v_min, float v_max, ImStr display_format, float power)
{
    return DragFloatN(label, v, 4, v_speed, v_min, v_max, display_format, power);
}

bool ImGui::DragFloatRange2(ImStr label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, ImStr display_format, ImStr display_format_max, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    value_changed |= DragFloat("##max", v_current_max, v_speed, (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min), (v_min >= v_max) ? FLT_MAX : v_max, display_format_max.Begin ? display_format_max : display_format, power);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragInt(ImStr label, int* v, float v_speed, int v_min, int v_max, ImStr display_format)
{
    if (!display_format.Begin)
        display_format.Begin = "%.0f";
    float v_f = (float)*v;
...
bool ImGui::DragIntN(ImStr label, int* v, int components, float v_speed, int v_min, int v_max, ImStr display_format)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::DragInt2(ImStr label, int v[2], float v_speed, int v_min, int v_max, ImStr display_format)
{
    return DragIntN(label, v, 2, v_speed, v_min, v_max, display_format);
}

bool ImGui::DragInt3(ImStr label, int v[3], float v_speed, int v_min, int v_max, ImStr display_format)
{
    return DragIntN(label, v, 3, v_speed, v_min, v_max, display_format);
}

bool ImGui::DragInt4(ImStr label, int v[4], float v_speed, int v_min, int v_max, ImStr display_format)
{
    return DragIntN(label, v, 4, v_speed, v_min, v_max, display_format);
}

bool ImGui::DragIntRange2(ImStr label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, ImStr display_format, ImStr display_format_max)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    value_changed |= DragInt("##max", v_current_max, v_speed, (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min), (v_min >= v_max) ? INT_MAX : v_max, display_format_max.Begin ? display_format_max : display_format);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::InputTextEx(ImStr label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (g.ActiveId == id || is_currently_scrolling)
    {
        ...
        draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos - render_scroll, GetColorU32(ImGuiCol_Text), ImStr(buf_display, buf_display + edit_state.CurLenA), 0.0f, is_multiline ? NULL : &clip_rect);
    ...
    else
    {
        ...
        draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos, GetColorU32(ImGuiCol_Text), ImStr(buf_display, buf_end), 0.0f, is_multiline ? NULL : &clip_rect);
    }
    ...
    if (g.LogEnabled && !is_password)
        LogRenderedText(&render_pos, buf_display);
...
bool ImGui::InputText(ImStr label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, buf, (int)buf_size, ImVec2(0,0), flags, callback, user_data);
}

bool ImGui::InputTextMultiline(ImStr label, char* buf, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
...
bool ImGui::InputScalarEx(ImStr label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, ImStr scalar_format, ImGuiInputTextFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (InputText("", buf, IM_ARRAYSIZE(buf), extra_flags)) // PushId(label) + "" gives us the expected ID from outside point of view
        value_changed = DataTypeApplyOpFromText(buf, GImGui->InputTextState.InitialText.begin(), data_type, data_ptr, fmt_buf);
...
bool ImGui::InputFloat(ImStr label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
...
bool ImGui::InputInt(ImStr label, int* v, int step, int step_fast, ImGuiInputTextFlags extra_flags)
{
...
bool ImGui::InputFloatN(ImStr label, float* v, int components, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::InputFloat2(ImStr label, float v[2], int decimal_precision, ImGuiInputTextFlags extra_flags)
{
    return InputFloatN(label, v, 2, decimal_precision, extra_flags);
}

bool ImGui::InputFloat3(ImStr label, float v[3], int decimal_precision, ImGuiInputTextFlags extra_flags)
{
    return InputFloatN(label, v, 3, decimal_precision, extra_flags);
}

bool ImGui::InputFloat4(ImStr label, float v[4], int decimal_precision, ImGuiInputTextFlags extra_flags)
{
    return InputFloatN(label, v, 4, decimal_precision, extra_flags);
}

bool ImGui::InputIntN(ImStr label, int* v, int components, ImGuiInputTextFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::InputInt2(ImStr label, int v[2], ImGuiInputTextFlags extra_flags)
{
    return InputIntN(label, v, 2, extra_flags);
}

bool ImGui::InputInt3(ImStr label, int v[3], ImGuiInputTextFlags extra_flags)
{
    return InputIntN(label, v, 3, extra_flags);
}

bool ImGui::InputInt4(ImStr label, int v[4], ImGuiInputTextFlags extra_flags)
{
...
bool ImGui::BeginCombo(ImStr label, ImStr preview_value, ImVec2 popup_size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (preview_value.Begin != NULL)
        RenderTextClipped(frame_bb.Min + style.FramePadding, value_bb.Max, preview_value, NULL, ImVec2(0.0f,0.0f));
...
bool ImGui::Selectable(ImStr label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    RenderTextClipped(bb.Min, bb_with_spacing.Max, label, &label_size, ImVec2(0.0f,0.0f));
...
bool ImGui::MenuItem(ImStr label, ImStr shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        if (shortcut_size.x > 0.0f)
        {
            ...
            RenderText(pos + ImVec2(window->MenuColumns.Pos[1] + extra_w, 0.0f), shortcut, false);
            PopStyleColor();
        }
        if (selected)
...
void ImGui::ColorTooltip(ImStr text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    text.End = text.Begin ? FindRenderedTextEnd(text) : text.End;
...
void ImGui::BeginColumns(ImStr id, int columns_count, ImGuiColumnsFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    PushID(0x11223347 + (id.Begin ? 0 : columns_count));
    window->DC.ColumnsSetId = window->GetID(id.Begin ? id : "columns");
    PopID();
...
void ImGui::Value(ImStr prefix, bool b)
{
    ...
    Text("%.*s: %s", size, prefix.Begin, (b ? "true" : "false"));
}

void ImGui::Value(ImStr prefix, int v)
{
    const int size = prefix.End ? (int)(prefix.End - prefix.Begin) : strlen(prefix.Begin);
    Text("%.*s: %d", size, prefix.Begin, v);
}
