...
// FIXME-OPT: Replace with e.g. FNV1a hash? CRC32 pretty much randomly access 1KB. Need to do proper measurements.
ImU32 ImHashStr(ImStr str, ImU32 seed)
{
    seed = ~seed;
    ImU32 crc = seed;
...
ImFileHandle ImFileOpen(ImStr filename, ImStr mode)
{
    ...
    const int filename_wsize = ::MultiByteToWideChar(CP_UTF8, 0, filename.Begin, IM_IMSTR_LENGTH(filename), NULL, 0);
    const int mode_wsize = ::MultiByteToWideChar(CP_UTF8, 0, mode.Begin, IM_IMSTR_LENGTH(mode), NULL, 0);
    ImVector<ImWchar> buf;
    buf.resize(filename_wsize + mode_wsize);
    ::MultiByteToWideChar(CP_UTF8, 0, filename.Begin, IM_IMSTR_LENGTH(filename), (wchar_t*)&buf[0], filename_wsize);
    ::MultiByteToWideChar(CP_UTF8, 0, mode.Begin, IM_IMSTR_LENGTH(mode), (wchar_t*)&buf[filename_wsize], mode_wsize);
...
ImGuiWindow::ImGuiWindow(ImGuiContext* context, ImStr name)
...
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    window->IDStack.push_back(window->GetIDNoKeepAlive(ImStr(str_id_begin, str_id_end)));
}
...
bool ImGui::OpenPopupOnItemClick(ImStr str_id, ImGuiMouseButton mouse_button)
{
    ImGuiWindow* window = GImGui->CurrentWindow;
...
bool ImGui::BeginPopupContextItem(ImStr str_id, ImGuiMouseButton mouse_button)
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    if (window->SkipItems)
        return false;
...
bool ImGui::BeginPopupContextWindow(ImStr str_id, ImGuiMouseButton mouse_button, bool also_over_items)
{
    if (!str_id)
...
bool ImGui::BeginPopupContextVoid(ImStr str_id, ImGuiMouseButton mouse_button)
{
    if (!str_id)
        str_id = "void_context";
    ImGuiID id = GImGui->CurrentWindow->GetID(str_id);
