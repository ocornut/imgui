...
namespace ImGui
{
    ...
    // Window
    IMGUI_API bool          Begin(ImStr name, bool* p_open = NULL, ImGuiWindowFlags flags = 0);                                                   // push window to the stack and start appending to it. see .cpp for details. return false when window is collapsed, so you can early out in your code. 'bool* p_open' creates a widget on the upper-right to close the window (which sets your bool to false).
    ...
    // Widgets: Text
    IMGUI_API void          TextUnformatted(ImStr text);               // doesn't require null terminated string if 'text_end' is specified. no copy done, no limits, recommended for long chunks of text
    IMGUI_API void          TextUnformatted(const char* text, const char* text_end);               // doesn't require null terminated string if 'text_end' is specified. no copy done, no limits, recommended for long chunks of text
    ...
    // Widgets: Main
    IMGUI_API bool          Button(ImStr label, const ImVec2& size = ImVec2(0,0));            // button
    ...
    IMGUI_API bool          Combo(ImStr label, int* current_item, ImStr const* items, int items_count, int height_in_items = -1);
    IMGUI_API bool          Combo(ImStr label, int* current_item, const char* const* items, int items_count, int height_in_items = -1);
    ...
    IMGUI_API bool          SliderFloat(ImStr label, float* v, float v_min, float v_max, ImStr display_format = "%.3f", float power = 1.0f);     // adjust display_format to decorate the value with a prefix or a suffix for in-slider labels or unit display. Use power!=1.0 for logarithmic sliders
    ...
    // Widgets: Trees
    IMGUI_API bool          TreeNode(ImStr label);                                            // if returning 'true' the node is open and the tree id is pushed into the id stack. user is responsible for calling TreePop().
    IMGUI_API bool          TreeNode(ImStr str_id, ImStr text);
    IMGUI_API bool          TreeNode(ImStr str_id, const char* fmt, ...) IM_FMTARGS(2);       // read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
    ...
    IMGUI_API void          TreePush(ImStr str_id = NULL);                                    // ~ Indent()+PushId(). Already called by TreeNode() when returning true, but you can call Push/Pop yourself for layout purpose
    ...
    IMGUI_API bool          ListBox(ImStr label, int* current_item, ImStr const* items, int items_count, int height_in_items = -1);
    ...
    // Popups
    IMGUI_API void          OpenPopup(ImStr str_id);                                      // call to mark popup as open (don't call every frame!). popups are closed when user click outside, or if CloseCurrentPopup() is called within a BeginPopup()/EndPopup() block. By default, Selectable()/MenuItem() are calling CloseCurrentPopup(). Popup identifiers are relative to the current ID-stack (so OpenPopup and BeginPopup needs to be at the same level).
    IMGUI_API bool          OpenPopupOnItemClick(ImStr str_id = NULL, int mouse_button = 1);                                  // helper to open popup when clicked on last item. return true when just opened.
    IMGUI_API bool          BeginPopup(ImStr str_id);                                     // return true if the popup is open, and you can start outputting to it. only call EndPopup() if BeginPopup() returned true!
    IMGUI_API bool          BeginPopupModal(ImStr name, bool* p_open = NULL, ImGuiWindowFlags extra_flags = 0);               // modal dialog (block interactions behind the modal window, can't close the modal window by clicking outside)
    IMGUI_API bool          BeginPopupContextItem(ImStr str_id = NULL, int mouse_button = 1);                                 // helper to open and begin popup when clicked on last item. if you can pass a NULL str_id only if the previous item had an id. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
    IMGUI_API bool          BeginPopupContextWindow(ImStr str_id = NULL, int mouse_button = 1, bool also_over_items = true);  // helper to open and begin popup when clicked on current window.
    IMGUI_API bool          BeginPopupContextVoid(ImStr str_id = NULL, int mouse_button = 1);                                 // helper to open and begin popup when clicked in void (where there are no imgui windows).
    IMGUI_API void          EndPopup();
    IMGUI_API bool          IsPopupOpen(ImStr str_id);                                    // return true if the popup is open
...
struct ImGuiIO
{
    ...
    IMGUI_API void AddInputCharactersUTF8(ImStr utf8_chars);            // Add new characters into InputCharacters[] from an UTF-8 string
...
namespace ImGui
{
    bool                    Begin(ImStr name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha_override = -1.0f, ImGuiWindowFlags flags = 0); // OBSOLETE 1.52+. use SetNextWindowSize() instead if you want to set a window size.
