...
namespace ImGui
{
    ...
    IMGUI_API bool          OpenPopup(ImStrv str_id, ImGuiPopupFlags popup_flags = 0);                          // call to mark popup as open (don't call every frame!).
    IMGUI_API bool          OpenPopup(ImGuiID id, ImGuiPopupFlags popup_flags = 0);                             // id overload to facilitate calling from nested stacks
    IMGUI_API bool          OpenPopupOnItemClick(ImStrv str_id = ImStrv(), ImGuiPopupFlags popup_flags = 0);     // helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
...
struct ImGuiTextFilter
{
    ...
    IMGUI_API bool      PassFilter(ImStrv label) const;
    ...
    IMGUI_API bool      Draw(ImStrv label = "Filter");
    IMGUI_API bool      DrawWithHint(ImStrv label = "Filter", ImStrv hint = "incl -excl");
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    inline bool         Draw(ImStrv label, float width)                                 { if (width != 0.0f) ImGui::SetNextItemWidth(width); return Draw(label); }
