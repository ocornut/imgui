...
void* hook_LoadAsset_718(void* x0, void* x1, void* x2, void* x3, int x4) { 
    g_debug_hook_LoadAsset_718_count++; // [新增] 调用计数器
    if (x1 && x2) {
...
void* hook_LoadMapImpl(void* x0, void* bundlePath, void* assetName, void* container, uint64_t isBackground, uint64_t isMineMap) { 
    g_debug_hook_LoadMapImpl_count++; // [新增] 调用计数器
...
void DrawDebugWindow(const GameSnapshot* snap) {
    ...
    if (ImGui::Begin((const char*)u8"调试日志 & 全功能偏移链监控", &g_show_debug_window)) {
        ...
        if (ImGui::BeginTabBar("DebugTabs")) {
            ...
            if (ImGui::BeginTabItem((const char*)u8"底层函数调用 & 核心偏移")) {
                ImGui::BeginChild("FuncOffsetScroll", ImVec2(0, 0), true, ImGuiWindowFlags_AlwaysVerticalScrollbar);
                
                ImGui::TextColored(ImVec4(0.4f, 1.0f, 0.4f, 1.0f), (const char*)u8"【核心 Hook 调用监控 (判断函数特征是否失效)】");
                ImGui::Columns(2, "hook_columns", false);
                ImGui::Text("3Cd6058 (Row): %d", g_debug_hook_3Cd6058_count);
                ImGui::Text("set_Money: %d", g_debug_hook_set_Money_count);
                ImGui::Text("set_Level: %d", g_debug_hook_set_Level_count);
                ImGui::Text("TurnStart: %d", g_debug_TurnStartCalled);
                ImGui::Text("IsGameEnd: %d", g_debug_set_IsGameEnd_count);
                ImGui::Text("UpdateTurnStart: %d", g_debug_UpdateTurnStart_count);
                ImGui::NextColumn();
                ImGui::Text("InitTurnDropCfg: %d", g_debug_hook_InitTurnDropCfg_count);
                ImGui::Text("6A67e48 (Pool): %d", g_debug_hook_6A67e48_count);
                ImGui::Text("46cce08 (ShopSlot): %d", g_debug_hook_46cce08_count);
                ImGui::Text("4848cec (Refresh): %d", g_debug_hook_4848cec_count);
                ImGui::Text("LoadAsset: %d", g_debug_hook_LoadAsset_718_count);
                ImGui::Text("LoadMap: %d", g_debug_hook_LoadMapImpl_count);
                ImGui::Columns(1);
                
                ImGui::Separator();
                ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.2f, 1.0f), (const char*)u8"【商店/备战席数据结构偏移链】");
                ImGui::Text("Shop Base: 0x%X  |  Bench Base: 0x%X", g_off_shop_base, g_off_bench_base);
                ImGui::Text("Base -> p20: 0x%X", g_off_shop_p20);
                ImGui::Text("p20 -> Entries: 0x%X", g_off_shop_entries);
                ImGui::Text("Entries Count: 0x%X", g_off_entries_count);
                ImGui::Text("Entries Head: 0x%X", g_off_entries_head);
                ImGui::Text("Entries Step: 0x%X", g_off_entries_step);
                ImGui::Text("Entry Value: 0x%X", g_off_entry_value);
                ImGui::Text("Value Slot: 0x%X", g_off_value_slot);
                ImGui::Text("Value p10: 0x%X", g_off_value_p10);
                ImGui::Text("p10 HeroID: 0x%X", g_off_p10_heroId);
                
                ImGui::Separator();
                ImGui::TextColored(ImVec4(0.2f, 0.8f, 1.0f, 1.0f), (const char*)u8"【全服公共牌库数据结构偏移链】");
                ImGui::Text("Pool Heroku: 0x%X", g_off_pool_heroku);
                ImGui::Text("Pool AHeroku: 0x%X", g_off_pool_Aheroku);
                ImGui::Text("Entries1: 0x%X | Count: 0x%X | Head: 0x%X | Step: 0x%X", g_off_pool_entries1, g_off_pool_e1_count, g_off_pool_e1_head, g_off_pool_e1_step);
                ImGui::Text("Entries2: 0x%X | Count: 0x%X | Head: 0x%X | Step: 0x%X", g_off_pool_entries2, g_off_pool_e2_count, g_off_pool_e2_head, g_off_pool_e2_step);
                ImGui::Text("Value2 ID: 0x%X | Rem: 0x%X | Tot: 0x%X", g_off_pool_v2_id, g_off_pool_v2_rem, g_off_pool_v2_tot);
