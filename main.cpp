...
static bool g_ShowLogWindow = true;   // 默认显示日志窗口
...
void hook_InitActorParams(void* x0, void* x1, void* x2, void* x3) {
    ...
    if (x0 != nullptr) {
        ...
        AddLog("[HOOK] x0+0x%x = %p", g_Offsets.x0_to_addr1, (void*)test_addr);
    } else {
...
void AutoFetchDeviceResolution() {
    ...
    while (fgets(buffer, sizeof(buffer), pipe) != NULL) {
        ...
        if (sscanf(buffer, "Physical size: %dx%d", &w, &h) == 2 ||
            ...
            AddLog("[Auto] 物理分辨率: %.0fx%.0f", g_Touch.physW, g_Touch.physH);
        }
    }
    pclose(pipe);
}
...
EGLBoolean hook_eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    ...
    if (!g_Initialized) {
        ...
        for (int i = 0; i < 4; i++) {
            if (access(systemFonts[i], R_OK) == 0) {
                ...
                g_FontLoaded = true;
                AddLog("[ImGui] 加载字体成功: %s", systemFonts[i]);
                break;
            }
        }
        g_Initialized = true;
    ...
    if (ImGui::Begin("IL2CPP 透视框架 (偏移可调)")) {
        ...
        if (ImGui::CollapsingHeader("基础校准")) {
            ...
            ImGui::Text("偏移 X:%.1f Y:%.1f", g_Touch.offsetX, g_Touch.offsetY);
        ...
        ImGui::Separator();
        ImGui::Checkbox("雷达", &show_radar); ImGui::SameLine();
        ImGui::Checkbox("调试器", &show_pointer_debugger); ImGui::SameLine();
        ImGui::Checkbox("日志窗口", &g_ShowLogWindow);
        ImGui::Separator();
    ...
    if (g_ShowLogWindow) DrawLogWindow();
    ...
    if (ImGui::Begin("指针断点排错器 (IL2CPP)")) {
        if (ImGui::CollapsingHeader("动态偏移修改面板", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            auto drawOffsetBtn = [](const char* label, uint32_t& val) {
                ImGui::PushID(&val);
                ImGui::TextColored(ImVec4(0.5f,0.8f,1.0f,1.0f), "%s -> %d (0x%X)", label, val, val);
            ...
            drawOffsetBtn("addr1 (x0+?)", g_Offsets.x0_to_addr1);
            drawOffsetBtn("英雄数组 (addr1+?)", g_Offsets.addr1_to_addr2);
            drawOffsetBtn("实体数据 (item+?)", g_Offsets.item_to_addr3);
            drawOffsetBtn("属性区 (addr3+?)", g_Offsets.addr3_to_addr4);
            drawOffsetBtn("坐标区 (addr3+?)", g_Offsets.addr3_to_addr5);
            drawOffsetBtn("小地图 (addr3+?)", g_Offsets.addr3_to_addr6);
        ...
        ImGui::TextColored(d.baseX0 ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1), "[起点] x0: 0x%lx", d.baseX0);
        ImGui::TextColored(d.addr1 ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1), " └─ [+%d] addr1: 0x%lx", off.x0_to_addr1, d.addr1);
        ImGui::TextColored(d.addr2 ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1), "     └─ [+%d] addr2: 0x%lx", off.addr1_to_addr2, d.addr2);
        if (d.addr2) {
            for (int i=0; i<15; i++) {
                if (!d.items[i].base) continue;
                char node[64]; snprintf(node, sizeof(node), "[%d] 0x%lx", i, d.items[i].base);
                if (ImGui::TreeNodeEx(node, ImGuiTreeNodeFlags_Framed)) {
                    ImGui::TextColored(d.items[i].addr3 ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1), " └─ [+%d] addr3: 0x%lx", off.item_to_addr3, d.items[i].addr3);
                    if (d.items[i].addr3) {
                        ImGui::TextColored(d.items[i].addr4 ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1), "     ├─ [+%d] addr4: 0x%lx", off.addr3_to_addr4, d.items[i].addr4);
                        ImGui::TextColored(d.items[i].addr5 ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1), "     ├─ [+%d] addr5: 0x%lx", off.addr3_to_addr5, d.items[i].addr5);
                        ImGui::TextColored(d.items[i].addr6 ? ImVec4(0,1,0,1) : ImVec4(1,0,0,1), "     └─ [+%d] addr6: 0x%lx", off.addr3_to_addr6, d.items[i].addr6);
                    }
                    ImGui::TreePop();
                }
            }
...
void DrawLogWindow() {
    ...
    if (!ImGui::Begin("调试日志", &g_ShowLogWindow)) {
        ImGui::End();
        return;
    }
...
void* DelayedHookThread(void*) {
    AddLog("[Init] 插件启动，开始 Hook 渲染与输入...");
    ...
    if (egl) {
        ...
        AddLog("[EGL] DobbyHook 结果: %d", ret);
    } else {
        AddLog("[EGL] 未找到 eglSwapBuffers");
    }
    ...
    if (ins) {
        int ret = DobbyHook(ins, (void*)hook_consume, (void**)&old_consume);
        AddLog("[Input] DobbyHook 结果: %d", ret);
    } else {
        AddLog("[Input] 未找到 InputConsumer::consume");
    }
    
    std::thread(DataWorkerThread).detach();
    AddLog("[Init] 等待游戏模块加载...");
    while (true) {
        ...
        if (base) {
            ...
            AddLog("[Game] libil2cpp.so 基址: 0x%lx", base);
            AddLog("[Game] InitActor 绝对地址: 0x%lx", g_HookAddr_InitActor);
            AddLog("[Game] ClearActor 绝对地址: 0x%lx", g_HookAddr_ClearActor);
            ...
            AddLog("[Game] Hook InitActor 结果: %d, ClearActor: %d", ret1, ret2);
            break;
        }
        sleep(1);
    }
    AutoFetchDeviceResolution();
    AddLog("[Init] 所有初始化完成，等待对局...");
    return nullptr;
}
