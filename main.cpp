...
EGLBoolean hook_eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    ...
    if (!g_Initialized) {
        ...
        // --- 核心修复：根据你的 fonts.xml 日志锁定 OPPO 系统字体 ---
        const char* mySystemFonts[] = {
            "/system/fonts/SysSans-Hans-Regular.ttf", // 你的日志里那个 OPPO Sans
            "/system/fonts/NotoSansCJKjp-Regular.otc", // 你的日志里那个 Noto CJK
            "/system/fonts/NotoSansSC-Regular.otf",
            "/system/fonts/DroidSansFallback.ttf"
        };
        for (int i = 0; i < 4; i++) {
            if (access(mySystemFonts[i], R_OK) == 0) {
                // 加载中文字符范围，解决问号问题
                io.Fonts->AddFontFromFileTTF(mySystemFonts[i], 32.0f, NULL, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
                g_FontLoaded = true;
                LOGI("[*] 成功锁定你的系统字体: %s", mySystemFonts[i]);
                break;
            }
        }
        g_Initialized = true;
    ...
    static bool show_pointer_debugger = true; static bool show_radar = true;
    ...
    if (show_radar && snapshot.inMatch) {
        ...
        if (ImGui::Begin("Radar", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize)) {
            ImVec2 p0 = ImGui::GetCursorScreenPos(); ImVec2 size = ImGui::GetContentRegionAvail();
            ...
            for (const auto& p : snapshot.players) {
                ...
                float rX = p0.x + (nX * size.x); float rY = p0.y + size.y - (nY * size.y); 
                if (nX > 0 && nX < 1 && nY > 0 && nY < 1) dl->AddCircleFilled(ImVec2(rX, rY), 5.0f * finalScale, IM_COL32(255, 0, 0, 255));
            }
        }
        ImGui::End(); ImGui::PopStyleColor();
    }
    ImGui::Render();
    GLint v[4]; glGetIntegerv(GL_VIEWPORT, v); GLint d; glGetIntegerv(GL_DEPTH_TEST, &d);
    glDisable(GL_DEPTH_TEST); glViewport(0, 0, w, h); 
    ImGui_ImplOpenGL3_RenderDrawData(ImGui::GetDrawData());
    glViewport(v[0], v[1], v[2], v[3]); if (d) glEnable(GL_DEPTH_TEST);
    return old_eglSwapBuffers(dpy, surface);
}

void* DelayedHookThread(void*) {
    // 1. 无延迟 Hook：渲染和输入必须秒挂，否则菜单出不来
