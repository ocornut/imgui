...
struct DynamicOffsets {
    ...
    uint32_t prop_maxhp = 0x32;   // int16
...
void AutoFetchDeviceResolution() {
    ...
    while (fgets(buffer, sizeof(buffer), pipe) != NULL) {
        ...
        if (sscanf(buffer, "Physical size: %dx%d", &w, &h) == 2 ||
            ...
            LOGI("[Auto] Physical resolution: %.0fx%.0f", g_Touch.physW, g_Touch.physH);
        }
    }
    pclose(pipe);
}
...
EGLBoolean hook_eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    ...
    if (!g_Initialized) {
        ...
        for (int i = 0; i < 4; i++) {
            if (access(systemFonts[i], R_OK) == 0) {
                ...
                g_FontLoaded = true;
                LOGI("[ImGui] Loaded font: %s", systemFonts[i]);
                break;
            }
        }
        g_Initialized = true;
    ...
    if (ImGui::Begin("IL2CPP 透视框架 (偏移可调)")) {
        ...
        ImGui::Checkbox("调试器", &show_pointer_debugger);
        ImGui::Separator();
...
void* DelayedHookThread(void*) {
    ...
    if (egl) {
        ...
        LOGI("[EGL] DobbyHook result: %d", ret);
    } else {
    ...
    if (ins) {
        ...
        LOGI("[Input] DobbyHook result: %d", ret);
    } else {
        LOGE("[Input] InputConsumer::consume not found");
    }

    // 2. 启动后台数据线程
    std::thread(DataWorkerThread).detach();

    // 3. 等待 libil2cpp.so 加载并 Hook 游戏函数
    while (true) {
        ...
        if (base != 0) {
            ...
            LOGI("[Game] libil2cpp.so base = 0x%lx", base);
            LOGI("[Game] InitActor absolute addr = 0x%lx", g_HookAddr_InitActor);
            LOGI("[Game] ClearActor absolute addr = 0x%lx", g_HookAddr_ClearActor);
            ...
            LOGI("[Game] DobbyHook InitActor result: %d (0=success)", ret1);
            LOGI("[Game] DobbyHook ClearActor result: %d", ret2);

            if (ret1 == 0 && ret2 == 0) {
                LOGI("[Game] Hooks installed successfully");
            } else {
                LOGE("[Game] Hook installation failed");
            }
            break;
        }
        sleep(1);
