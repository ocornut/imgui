...
std::atomic<uintptr_t> g_debug_pool_base{0}; // 新增: 记录池基址以供显示
...
void* hook_6A67e48(void* x0, void* x1, void* x2, void* x3) {
    ...
    if (x0 != nullptr) {
        ...
        g_debug_pool_base.store(base, std::memory_order_relaxed); // 修改: 记录池基址
...
void ColoredSeparator() { 
    ...
    float pad = 4.0f * g_autoScale * g_scale;
    ImRect bb(win->DC.CursorPos, win->DC.CursorPos + ImVec2(ImGui::GetContentRegionAvail().x, pad * 2.0f + 1.0f));
    ImGui::ItemSize(bb); if (!ImGui::ItemAdd(bb, 0)) return;
    ImDrawList* dl = win->DrawList;
    float w = bb.GetWidth();
    ImU32 fadeL = IM_COL32(0, 243, 255, 0);
    ImU32 fadeM1 = IM_COL32(0, 243, 255, 180);
    ImU32 fadeM2 = IM_COL32(255, 0, 128, 180);
    ImU32 fadeR = IM_COL32(255, 0, 128, 0);
    dl->AddRectFilledMultiColor(bb.Min + ImVec2(0, pad), bb.Min + ImVec2(w * 0.35f, pad + 1.0f), fadeL, fadeM1, fadeM1, fadeL);
    dl->AddRectFilledMultiColor(bb.Min + ImVec2(w * 0.35f, pad), bb.Min + ImVec2(w * 0.65f, pad + 1.0f), fadeM1, fadeM2, fadeM2, fadeM1);
    dl->AddRectFilledMultiColor(bb.Min + ImVec2(w * 0.65f, pad), bb.Min + ImVec2(w, pad + 1.0f), fadeM2, fadeR, fadeR, fadeM2);
}
...
static void ApplyMenuTheme(ImGuiStyle& style) {
    ...
    style.ItemSpacing = ImVec2(10.0f * s, 10.0f * s);
    ...
    style.GrabMinSize = 20.0f * s;
    style.TabBorderSize = 0.0f;

    ImVec4* c = style.Colors;
    c[ImGuiCol_Text]                  = ImVec4(0.92f, 0.96f, 1.0f, 1.0f);
    c[ImGuiCol_TextDisabled]          = ImVec4(0.40f, 0.50f, 0.65f, 1.0f);
...
static void DrawMenuHeaderBanner(ImDrawList* dl, ImVec2 winPos, float winW, float fps) {
    float h = 52.0f * g_autoScale * g_scale;
    ImVec2 p0 = winPos;
    ImVec2 p1 = winPos + ImVec2(winW, h);
    dl->AddRectFilled(p0, p1, IM_COL32(14, 16, 22, 255), 14.0f * g_autoScale, ImDrawFlags_RoundCornersTop);
    dl->AddRectFilledMultiColor(p0, p1,
        IM_COL32(0, 180, 140, 35), IM_COL32(0, 120, 200, 25),
        IM_COL32(0, 120, 200, 10), IM_COL32(0, 180, 140, 35));
    dl->AddLine(p0 + ImVec2(16.0f * g_autoScale, h - 1.0f), p0 + ImVec2(winW - 16.0f * g_autoScale, h - 1.0f),
        IM_COL32(0, 212, 170, 60), 1.0f);

    ImFont* font = g_mainFont ? g_mainFont : ImGui::GetFont();
    float titleSz = 22.0f * g_autoScale * g_scale;
    const char* title = (const char*)u8"金铲铲助手";
    ImVec2 titleSz2 = font->CalcTextSizeA(titleSz, FLT_MAX, 0.0f, title);
    dl->AddText(font, titleSz, p0 + ImVec2(18.0f * g_autoScale, (h - titleSz2.y) * 0.5f - 2.0f),
        IM_COL32(0, 212, 170, 255), title);

    float pulse = (sinf((float)ImGui::GetTime() * 3.0f) + 1.0f) * 0.5f;
    dl->AddCircleFilled(p0 + ImVec2(winW - 90.0f * g_autoScale, h * 0.5f), 4.0f * g_autoScale,
        IM_COL32(0, (int)(180 + 75 * pulse), (int)(140 + 60 * pulse), 255));
    char fpsBuf[32]; snprintf(fpsBuf, sizeof(fpsBuf), "%.0f FPS", fps);
    float fpsSz = 14.0f * g_autoScale * g_scale;
    ImVec2 fpsTs = font->CalcTextSizeA(fpsSz, FLT_MAX, 0.0f, fpsBuf);
    dl->AddText(font, fpsSz, p0 + ImVec2(winW - 82.0f * g_autoScale, (h - fpsTs.y) * 0.5f),
        IM_COL32(140, 145, 160, 255), fpsBuf);
}

static bool BeginMenuSection(const char* title) {
    ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0.13f, 0.14f, 0.19f, 0.90f));
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(14.0f * g_autoScale * g_scale, 12.0f * g_autoScale * g_scale));
    char childId[64]; snprintf(childId, sizeof(childId), "##sec_%s", title);
    ImGui::BeginChild(childId, ImVec2(-1, 0), false, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoScrollbar);
    ImGui::TextColored(ImVec4(0.0f, 0.83f, 0.67f, 1.0f), "%s", title);
    ImGui::Spacing();
    return true;
}

static void EndMenuSection() {
    ImGui::EndChild();
    ImGui::PopStyleVar();
    ImGui::PopStyleColor();
    ImGui::Spacing();
...
bool ModernAnimatedFolder(const char* label, bool* state) {
    ImGuiWindow* win = ImGui::GetCurrentWindow(); ImGuiID id = win->GetID(label); ImVec2 pos = win->DC.CursorPos; ImVec2 size(ImGui::GetContentRegionAvail().x, ImGui::GetFrameHeight() * 1.35f);
...
void ModernTierSelector(const char* label, bool* tierArray) {
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        ImVec2 pos = win->DC.CursorPos; ImRect bb(pos, pos + ImVec2(ImGui::GetFrameHeight()*1.4f, ImGui::GetFrameHeight())); ImGui::ItemSize(bb);
        ...
        win->DrawList->AddText(ImGui::GetFont(), ImGui::GetFontSize(), pos + ImVec2((bb.GetWidth() - t_sz.x)*0.5f, (bb.GetHeight() - t_sz.y)*0.5f), 
            tierArray[i] ? IM_COL32(0, 243, 255, 255) : IM_COL32(100, 120, 145, 255), buf);
    }
}
...
void DrawDebugWindow(const GameSnapshot* snap) {
    ...
    if (ImGui::Begin((const char*)u8"调试日志 & 全功能偏移链监控", &g_show_debug_window)) {
        ...
        if (ImGui::BeginTabBar("DebugTabs")) {
            ...
            if (ImGui::BeginTabItem((const char*)u8"基址与调用次数监控")) {
                ImGui::BeginChild("BaseAndCallScroll", ImVec2(0, 0), true, ImGuiWindowFlags_AlwaysVerticalScrollbar);
                
                ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.2f, 1.0f), (const char*)u8"【核心基址】");
                ImGui::Text("il2cpp Base: 0x%lX", g_il2cppBase);
                ImGui::Text("BattleModel: 0x%lX", snap->chessBattleModel);
                ImGui::Text("CSOGame: 0x%lX", snap->csoGame);
                ImGui::Text("Pool Base: 0x%lX", g_debug_pool_base.load());
                ImGui::Text("Main Camera: 0x%lX", snap->mainCamera);
                
                ImGui::Separator();
                ImGui::TextColored(ImVec4(0.2f, 0.8f, 1.0f, 1.0f), (const char*)u8"【函数调用计数 (Hook监控)】");
                ImGui::Text("hook_3Cd6058: %d", g_debug_hook_3Cd6058_count);
                ImGui::Text("hook_set_Money: %d", g_debug_hook_set_Money_count);
                ImGui::Text("hook_set_Level: %d", g_debug_hook_set_Level_count);
                ImGui::Text("hook_TurnStart: %d", g_debug_TurnStartCalled);
                ImGui::Text("hook_set_IsGameEnd: %d", g_debug_set_IsGameEnd_count);
                ImGui::Text("hook_UpdateTurnStart: %d", g_debug_UpdateTurnStart_count);
                ImGui::Text("hook_InitTurnDropCfg: %d", g_debug_hook_InitTurnDropCfg_count);
                ImGui::Text("hook_6A67e48 (Pool): %d", g_debug_hook_6A67e48_count);
                ImGui::Text("hook_SendWillRenderCanvases: %d", g_debug_SendWillRenderCanvases_count);
                ImGui::Text("Background Threads Run: %d", g_debug_bg_thread_count);
                
                ImGui::EndChild();
                ImGui::EndTabItem();
            }

            if (ImGui::BeginTabItem((const char*)u8"动态偏移链监控")) {
                ImGui::BeginChild("OffsetChainScroll", ImVec2(0, 0), true, ImGuiWindowFlags_AlwaysVerticalScrollbar);
                
                auto myIt = snap->players.find(snap->myPlayerId);
                uintptr_t myObj = (myIt != snap->players.end()) ? myIt->second.objAddr : 0;
                uintptr_t myLogicObj = (myIt != snap->players.end()) ? myIt->second.logicObjAddr : 0;
                
                ImGui::TextColored(ImVec4(0.2f, 1.0f, 0.5f, 1.0f), (const char*)u8"【我方商店链】");
                if (myObj) {
                    ...
                    ImGui::Text("PlayerObj: 0x%lX", myObj);
                    ImGui::Text(" -> +0x%X (Base): 0x%lX", g_off_shop_base, pBase);
                    ImGui::Text(" -> +0x%X (p20): 0x%lX", g_off_shop_p20, p20);
                    ImGui::Text(" -> +0x%X (Entries): 0x%lX", g_off_shop_entries, entries);
                } else {
                    ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), (const char*)u8"PlayerObj Not Found (等待进入对局)");
                }
                ImGui::Separator();

                ImGui::TextColored(ImVec4(0.2f, 1.0f, 0.5f, 1.0f), (const char*)u8"【我方备战席链】");
                if (myObj) {
                    uintptr_t pBase = SAFE_READ_PTR(myObj, g_off_bench_base);
                    uintptr_t p20 = pBase ? SAFE_READ_PTR(pBase, g_off_shop_p20) : 0;
                    uintptr_t entries = p20 ? SAFE_READ_PTR(p20, g_off_shop_entries) : 0;
                    ImGui::Text("PlayerObj: 0x%lX", myObj);
                    ImGui::Text(" -> +0x%X (Base): 0x%lX", g_off_bench_base, pBase);
                    ImGui::Text(" -> +0x%X (p20): 0x%lX", g_off_shop_p20, p20);
                    ImGui::Text(" -> +0x%X (Entries): 0x%lX", g_off_shop_entries, entries);
                } else {
                    ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), (const char*)u8"PlayerObj Not Found (等待进入对局)");
                ...
                uintptr_t poolBase = g_debug_pool_base.load();
                if (poolBase) {
                    uintptr_t heroku = SAFE_READ_PTR(poolBase, g_off_pool_heroku);
                    ...
                    ImGui::Text("PoolBase: 0x%lX", poolBase);
                    ImGui::Text(" -> +0x%X (Heroku): 0x%lX", g_off_pool_heroku, heroku);
                    ImGui::Text(" -> +0x%X (AHeroku): 0x%lX", g_off_pool_Aheroku, aheroku);
                    ImGui::Text(" -> +0x%X (Entries1): 0x%lX", g_off_pool_entries1, entries1);
                } else {
                    ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), (const char*)u8"PoolBase Not Found (等待调用 hook_6A67e48)");
                }
                ImGui::Separator();
                ...
                if (myLogicObj) {
                    ...
                    ImGui::Text("LogicObj: 0x%lX", myLogicObj);
                    ImGui::Text(" -> +0x%X (EquipBase): 0x%lX", g_off_equip_base, pEquipBase);
                    ImGui::Text(" -> +0x%X (EquipArr): 0x%lX", g_off_equip_arr, itemArr);
                } else {
                    ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), (const char*)u8"LogicObj Not Found (等待进入对局)");
                }
                ImGui::Separator();
                ...
                if (myLogicObj) {
                    ...
                    ImGui::Text("LogicObj: 0x%lX", myLogicObj);
                    ImGui::Text(" -> +0x%X (HeroDyn): 0x%lX", g_off_hero_dyn, pDyn);
                    ImGui::Text(" -> +0x%X (HeroArr): 0x%lX", g_off_hero_arr, heroArr);
                } else {
                    ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), (const char*)u8"LogicObj Not Found (等待进入对局)");
                }
    ...
    if (ImGui::Begin((const char*)u8"金铲铲助手 // CYBER-HUD", NULL, ImGuiWindowFlags_NoSavedSettings)) {
        ...
        if (!g_menuCollapsed) {
            ...
            // ===============================================
            {
                ...
                // --- 基址第1行：标题 + 展开按钮 ---
                ImGui::TextColored(ImVec4(0.55f, 0.55f, 0.55f, 1.0f), (const char*)u8"■ 基址监控");
                ImGui::SameLine(0, 10);
                ...
                // --- 展开区域（带滚动条） ---
                if (s_mon_open) {
                    float expandH = 420.0f * g_autoScale;
                    ImGui::Separator();
                    ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0.05f,0.07f,0.10f,0.80f));
                    ImGui::BeginChild("MonScrollArea", ImVec2(-1, expandH), false,
                    ...
                    ImGui::Columns(2, "HookCols", false);
                    ...
                    uintptr_t myObj2 = 0, myLogicObj2 = 0;
                    if (snapBase) {
                        auto myIt2 = snapBase->players.find(snapBase->myPlayerId);
                        if (myIt2 != snapBase->players.end()) {
                            myObj2      = myIt2->second.objAddr;
                            myLogicObj2 = myIt2->second.logicObjAddr;
                        }
                    }
                    uintptr_t poolB2 = g_debug_pool_base.load();

                    // 辅助：地址若非零显示绿色，否则灰色
                    auto AddrLine = [&](const char* indent, const char* label, uintptr_t addr) {
                        if (addr)
                            ImGui::TextColored(ImVec4(0.3f,1.0f,0.6f,1.0f), "%s%s: 0x%lX", indent, label, addr);
                        else
                            ImGui::TextColored(ImVec4(0.5f,0.5f,0.5f,0.7f), "%s%s: (null)", indent, label);
                    };
                    ...
                    if (myObj2) {
                        uintptr_t pBase   = SAFE_READ_PTR(myObj2, g_off_shop_base);
                        uintptr_t p20     = pBase ? SAFE_READ_PTR(pBase, g_off_shop_p20)    : 0;
                        uintptr_t entries = p20   ? SAFE_READ_PTR(p20,   g_off_shop_entries) : 0;
                        AddrLine("  ", "PlayerObj", myObj2);
                        ImGui::Text("   +0x%X ->", g_off_shop_base); ImGui::SameLine(0,4); AddrLine("","Base",pBase);
                        ImGui::Text("   +0x%X ->", g_off_shop_p20);  ImGui::SameLine(0,4); AddrLine("","p20", p20);
                        ImGui::Text("   +0x%X ->", g_off_shop_entries); ImGui::SameLine(0,4); AddrLine("","Entries",entries);
                    ...
                    if (myObj2) {
                        uintptr_t pBase   = SAFE_READ_PTR(myObj2, g_off_bench_base);
                        uintptr_t p20     = pBase ? SAFE_READ_PTR(pBase, g_off_shop_p20)    : 0;
                        uintptr_t entries = p20   ? SAFE_READ_PTR(p20,   g_off_shop_entries) : 0;
                        AddrLine("  ", "PlayerObj", myObj2);
                        ImGui::Text("   +0x%X ->", g_off_bench_base); ImGui::SameLine(0,4); AddrLine("","Base",pBase);
                        ImGui::Text("   +0x%X ->", g_off_shop_p20);   ImGui::SameLine(0,4); AddrLine("","p20", p20);
                        ImGui::Text("   +0x%X ->", g_off_shop_entries); ImGui::SameLine(0,4); AddrLine("","Entries",entries);
                    } else { ImGui::TextColored(ImVec4(0.5f,0.5f,0.5f,0.7f), (const char*)u8"  等待对局..."); }
                    ...
                    ImGui::TextColored(ImVec4(1.0f,0.5f,0.2f,1.0f), (const char*)u8"▶ 卡池链 (Pool)");
                    if (poolB2) {
                        uintptr_t heroku   = SAFE_READ_PTR(poolB2, g_off_pool_heroku);
                        uintptr_t aheroku  = heroku  ? SAFE_READ_PTR(heroku,  g_off_pool_Aheroku)  : 0;
                        uintptr_t entries1 = aheroku ? SAFE_READ_PTR(aheroku, g_off_pool_entries1) : 0;
                        AddrLine("  ", "PoolBase", poolB2);
                        ImGui::Text("   +0x%X ->", g_off_pool_heroku);   ImGui::SameLine(0,4); AddrLine("","Heroku", heroku);
                        ImGui::Text("   +0x%X ->", g_off_pool_Aheroku);  ImGui::SameLine(0,4); AddrLine("","AHeroku",aheroku);
                        ImGui::Text("   +0x%X ->", g_off_pool_entries1); ImGui::SameLine(0,4); AddrLine("","Entries1",entries1);
                    ...
                    ImGui::TextColored(ImVec4(1.0f,0.8f,0.2f,1.0f), (const char*)u8"▶ 装备链 (Equip)");
                    if (myLogicObj2) {
                        uintptr_t pEquipBase = SAFE_READ_PTR(myLogicObj2, g_off_equip_base);
                        uintptr_t itemArr    = pEquipBase ? SAFE_READ_PTR(pEquipBase, g_off_equip_arr) : 0;
                        AddrLine("  ", "LogicObj", myLogicObj2);
                        ImGui::Text("   +0x%X ->", g_off_equip_base); ImGui::SameLine(0,4); AddrLine("","EquipBase",pEquipBase);
                        ImGui::Text("   +0x%X ->", g_off_equip_arr);  ImGui::SameLine(0,4); AddrLine("","ItemArr",  itemArr);
                    } else { ImGui::TextColored(ImVec4(0.5f,0.5f,0.5f,0.7f), (const char*)u8"  等待对局..."); }
                    ...
                    ImGui::TextColored(ImVec4(0.8f,0.5f,1.0f,1.0f), (const char*)u8"▶ 英雄实体链 (Hero)");
                    if (myLogicObj2) {
                        uintptr_t pDyn    = SAFE_READ_PTR(myLogicObj2, g_off_hero_dyn);
                        uintptr_t heroArr = pDyn ? SAFE_READ_PTR(pDyn, g_off_hero_arr) : 0;
                        AddrLine("  ", "LogicObj", myLogicObj2);
                        ImGui::Text("   +0x%X ->", g_off_hero_dyn); ImGui::SameLine(0,4); AddrLine("","HeroDyn", pDyn);
                        ImGui::Text("   +0x%X ->", g_off_hero_arr); ImGui::SameLine(0,4); AddrLine("","HeroArr", heroArr);
                    } else { ImGui::TextColored(ImVec4(0.5f,0.5f,0.5f,0.7f), (const char*)u8"  等待对局..."); }
                ...
                ImGui::SetWindowFontScale(g_scale);
                ImGui::EndChild();
                ImGui::PopStyleColor(); // ChildBg outer
            }
            ColoredSeparator();
