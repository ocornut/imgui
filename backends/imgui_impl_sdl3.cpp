...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event)
{
    ...
    if (event->type == SDL_EVENT_WINDOW_CLOSE_REQUESTED || event->type == SDL_EVENT_WINDOW_MOVED || event->type == SDL_EVENT_WINDOW_RESIZED)
...
static bool ImGui_ImplSDL3_Init(SDL_Window* window, void* sdl_gl_context)
{
    g_Window = window;
...
bool ImGui_ImplSDL3_InitForOpenGL(SDL_Window* window, void* sdl_gl_context)
{
    ...
    return ImGui_ImplSDL3_Init(window, sdl_gl_context);
}
...
#endif
    g_UseVulkan = true;
    return ImGui_ImplSDL3_Init(window, NULL);
}
...
static void ImGui_ImplSDL3_UpdateMousePosAndButtons()
{
    ImGuiIO& io = ImGui::GetIO();
...
#endif
            SDL_WarpMouseInWindow(g_Window, io.MousePos.x, io.MousePos.y);
    }
    else
    {
...
static void ImGui_ImplSDL3_UpdateGamepads()
{
    ...
    #define MAP_BUTTON(NAV_NO, BUTTON_NO)       { io.NavInputs[NAV_NO] = (SDL_GetGamepadButton(game_controller, BUTTON_NO) != 0) ? 1.0f : 0.0f; }
    #define MAP_ANALOG(NAV_NO, AXIS_NO, V0, V1) { float vn = (float)(SDL_GetGamepadAxis(game_controller, AXIS_NO) - V0) / (float)(V1 - V0); if (vn > 1.0f) vn = 1.0f; if (vn > 0.0f && io.NavInputs[NAV_NO] < vn) io.NavInputs[NAV_NO] = vn; }
...
static void ImGui_ImplSDL3_UpdateMonitors()
{
    ...
    for (int n = 0; n < display_count; n++)
    {
        ...
        SDL_Rect r;
        SDL_GetDisplayBounds(displays[n], &r);
        ...
        float dpi = 0.0f;
        if (!SDL_GetDisplayPhysicalDPI(n, &dpi, NULL, NULL))
...
struct ImGuiViewportDataSDL3
{
    ...
    ImGuiViewportDataSDL3() { Window = NULL; WindowID = 0; WindowOwned = false; GLContext = NULL; }
    ~ImGuiViewportDataSDL3() { IM_ASSERT(Window == NULL && GLContext == NULL); }
};

static void ImGui_ImplSDL3_CreateWindow(ImGuiViewport* viewport)
{
    ImGuiViewportDataSDL3* data = IM_NEW(ImGuiViewportDataSDL3)();
    viewport->PlatformUserData = data;

    ImGuiViewport* main_viewport = ImGui::GetMainViewport();
    ImGuiViewportDataSDL3* main_viewport_data = (ImGuiViewportDataSDL3*)main_viewport->PlatformUserData;
    ...
    sdl_flags |= SDL_GetWindowFlags(g_Window);
...
#if defined(_WIN32)
    SDL_SysWMinfo info;
    if (SDL_GetWindowWMInfo(data->Window, &info, SDL_SYSWM_CURRENT_VERSION ))
...
{
    if (ImGuiViewportDataSDL3* data = (ImGuiViewportDataSDL3*)viewport->PlatformUserData)
    {
...
{
    ImGuiViewportDataSDL3* data = (ImGuiViewportDataSDL3*)viewport->PlatformUserData;
#if defined(_WIN32)
...
static void ImGui_ImplSDL3_InitPlatformInterface(SDL_Window* window, void* sdl_gl_context)
{
    ...
    // SDL3 by default doesn't pass mouse clicks to the application when the click focused a window. This is getting in the way of our interactions and we disable that behavior.
