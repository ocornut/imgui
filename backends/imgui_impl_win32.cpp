...
//  2021-08-07: Inputs: Added support for multiple gamepads.
...
static void ImGui_ImplWin32_UpdateGamepads()
{
    ...
    if (bd->WantUpdateHasGamepad)
    {
        for (int user = 0; user < 4; user++)
        {
            ...
            bd->HasGamepad[user] = bd->XInputGetCapabilities ? (bd->XInputGetCapabilities(user, XINPUT_FLAG_GAMEPAD, &caps) == ERROR_SUCCESS) : false;
        }
    ...
    for (int user = 0; user < 4; user++)
    {
        if (bd->HasGamepad[user] && bd->XInputGetState && bd->XInputGetState(user, &xinput_state) == ERROR_SUCCESS)
        {
            const XINPUT_GAMEPAD& gamepad = xinput_state.Gamepad;
            io.BackendFlags |= ImGuiBackendFlags_HasGamepad;

            #define MAP_BUTTON(NAV_NO, BUTTON_ENUM)     { if (gamepad.wButtons & BUTTON_ENUM) io.NavInputs[NAV_NO] = 1.0f; }
            #define MAP_ANALOG(NAV_NO, VALUE, V0, V1)   { float vn = (float)(VALUE - V0) / (float)(V1 - V0); if (vn > 1.0f) vn = 1.0f; if (vn > 0.0f) io.NavInputs[NAV_NO] += vn; }
