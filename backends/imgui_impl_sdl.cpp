    ...
    for (int i = 0; i < SDL_NumJoysticks(); ++i)
    {
        if (SDL_IsGameController(i))
        {
            ...
            if (game_controller)
            {
                ...
                #define MAP_BUTTON(NAV_NO, BUTTON_NO)       { if (SDL_GameControllerGetButton(game_controller, BUTTON_NO) != 0) io.NavInputs[NAV_NO] = 1.0f; }
                #define MAP_ANALOG(NAV_NO, AXIS_NO, V0, V1) { float vn = (float)(SDL_GameControllerGetAxis(game_controller, AXIS_NO) - V0) / (float)(V1 - V0); if (vn > 1.0f) vn = 1.0f; if (vn > 0.0f) io.NavInputs[NAV_NO] += vn; }
