...
#include "imgui_impl_sdlgpu_shaders.h"
...
static void CreateOrResizeBuffer(SDL_GPUBuffer** buffer, uint32_t new_size,SDL_GPUBufferUsageFlags usage)
{
...
static void ImGui_ImplSDLGPU_SetupRenderState(ImDrawData* draw_data, SDL_GPUCommandBuffer* command_buffer,SDL_GPURenderPass * render_pass, ImGui_ImplSDLGPU_FrameData* fd,uint32_t width, uint32_t height)
{
    ImGui_ImplSDLGPU_Data* bd = ImGui_ImplSDLGPU_GetBackendData();
...
void Imgui_ImplSDLGPU_PrepareDrawData(ImDrawData* draw_data,SDL_GPUCommandBuffer* command_buffer)
{
    ...
    if(draw_data->TotalVtxCount > 0)
    {
        ...
        if(fd->VertexBuffer == nullptr || fd->VertexBufferSize < vertex_size){
            CreateOrResizeBuffer(&fd->VertexBuffer,vertex_size, SDL_GPU_BUFFERUSAGE_VERTEX);
            fd->VertexBufferSize = vertex_size;
        }

        if (fd->IndexBuffer == nullptr || fd->IndexBufferSize < index_size){
            CreateOrResizeBuffer(&fd->IndexBuffer,index_size, SDL_GPU_BUFFERUSAGE_INDEX);
        ...
        IM_ASSERT(vertex_transferbuffer != nullptr && "Failed to create vertex transfer buffer, call SDL_GetError() for more information");
        ...
        IM_ASSERT(index_transferbuffer != nullptr && "Failed to create index transfer buffer, call SDL_GetError() for more information");

        ImDrawVert* vtx_dst = (ImDrawVert*)SDL_MapGPUTransferBuffer(v->GpuDevice,vertex_transferbuffer,cycle);
        ImDrawIdx*  idx_dst = (ImDrawIdx*)SDL_MapGPUTransferBuffer(v->GpuDevice,index_transferbuffer,cycle);
        ...
        SDL_UploadToGPUBuffer(copy_pass,&vertex_buffer_location,&vertex_buffer_region,cycle);
        SDL_UploadToGPUBuffer(copy_pass,&index_buffer_location,&index_buffer_region,cycle);
