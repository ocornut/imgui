...
Unfortunately it is still tedious to create and maintain portable build files using external
...
However, considering that OS mouse cursors are typically drawn through a very specific hardware accelerated
...
as the mouse, the disconnect may be jarring to particularly sensitive users.
...
Note:some setup configurations or GPU drivers may cause extra display lag depending on their settings.
If you feel that dragging windows feels laggy and you are not sure what the cause is:try
drawing a simple 2D shape directly under the mouse cursor to help identify the issue.
